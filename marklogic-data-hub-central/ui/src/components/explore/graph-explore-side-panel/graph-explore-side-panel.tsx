...
const GraphExploreSidePanel: React.FC<Props> = (props) => {
  ...
  const  {selectedNode, database, entityTypeIds, selectedFacets, query, sortOrder} = searchOptions;
  const {entityName, primaryKey, uri, sources, entityInstance} = selectedNode;
  ...
  return (
    <div id="sidePanel" className={styles.sidePanel}>
      <div>
        <span className={styles.selectedEntityHeading}>{entityName} {">"} {primaryKey.propertyValue}</span>
      ...
      <div>
        <span className={styles.selectedNodeUri}>URI: {uri}</span>
      </div>
